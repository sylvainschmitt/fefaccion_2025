...
<!-- @Géraldine, à quoi sert GGally ici? -->
...
```{r}
plot_traj_sp <- function(id, dt) {
    n <- dt %>% filter(sp == GenSpd) %>% count() # medições
    filter(dt, sp == GenSpd) %>% 
      ggplot(aes(x=DBH, y = Growth)) + 
      geom_point() + 
      geom_smooth(method = "loess", se = FALSE) + 
...
```{r}
#| eval: false
fit <- brm(bf(log(Growth+1) ~ G*exp(-0.5*(log(DBH/D)/K)^2),
              G ~ 1, D ~ 1, K ~ 1,
              nl = TRUE),
           prior = c(
...
```{r}
#| eval: false
fit_sp <- brm(bf(log(Growth+1) ~ G*exp(-0.5*(log(DBH/D)/K)^2),
              G ~ 1|GenSp, D ~ 1|GenSp, K ~ 1|GenSp, nl = TRUE),
                      prior = c(
                 prior("uniform(0.001,3)", lb = 0.001, ub = 3, nlpar = "G"),
                 prior("uniform(0.001,3)", lb = 0.001, ub = 3, nlpar = "K"),
                 prior("uniform(1,300)", lb = 1, ub = 300, nlpar = "D")
           ...
           data = filter(dt_P6, 
                         GenSp %in% c("Cecropia_obtusa", "Dicorynia_guianensis", 
                                      "Eschweilera_sagotiana")),
          cores = 4
           )
save(fit, file = "data/fit_sp.Rdata")
```

```{r}
load(file = "data/fit_sp.Rdata")
fit_sp
```
