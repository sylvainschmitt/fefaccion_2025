...
Usaremos os pacotes `tidyverse` e `GGally` para manipular tabelas e criar gráficos,  `vegan` para fazer análises da vegetação e `brms` para fazer modelização bayesiana.
...
plot_fit <- function(fit, Species) {
  ...
  filter(dt_P6, GenSp  == !!Species) %>% 
    ...
    geom_hline(yintercept = G, colour = "red", linetype = "dashed") +
...
fit %>% 
  ...
  labs(x = "Crescimento previsto (em cm/ano)")
```
