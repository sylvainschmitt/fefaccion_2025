...
Usaremos o `tidyverse` e o `GGally` para manipular tabelas e criar gráficos, o `vegan` para fazer análises da vegetação e o `brms` para fazer modelização bayesiana.
...
Começamos carregando os dados do gráfico na forma de objetos no formato Rdata.
...
Podemos começar examinando a distribuição de valores para cada um dos recursos com um histograma usando a biblioteca `ggplot'.
...
::: notes
CV igual à variância sigma dividida pela média mu
:::

## Correlações entre pares de características

As características não variam de forma independente, portanto, podemos explorar esses vínculos um a um usando os coeficientes de correlação.
...
Mas para entender melhor a organização geral de todos os recursos em “estratégias funcionais”, podemos construir uma análise de componente principal (ACP) com o pacote `vegan`.
...
Podemos representar a PCA em um biplot de eixos1-2 com as características como variáveis representadas por setas e as espécies por pontos posicionados de acordo com o velru de suas características.
...
Para usar os dados de crescimento, começamos filtrando os dados do lote 6.
...
::: notes
Podemos representar o crescimento como crescimento annual ao longo do tempo
:::
